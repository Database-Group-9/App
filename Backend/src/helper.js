...
function getRatingsSql(tagList, genreId){
    ...
    var intersectSql = format("SELECT COUNT(a.movieId), SUM(a.avgrating) FROM (SELECT movieid, avgrating FROM movies WHERE movieid IN (SELECT movieid FROM movie_genre WHERE genreid = %L", genreId[0])
    for(i = 1; i < tagList.length; i++){
    ...
    for(i = 1; i < tagList.length; i++){
        intersectSql = intersectSql.concat(format(" OR tag ILIKE %L", tagList[i]))
    }
